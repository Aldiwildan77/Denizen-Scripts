UnlockNatureboltQuestDeliveryHandler:
    type: task
    debug: false
    script:
    - define stage:1
    - define quest_internalname:UnlockNaturebolt
    - choose <player.item_in_hand.material.name>:
        ...
        - case default:
            ...
            - run QuestProgressHandler def:UnlockNaturebolt
            - stop
    - inject QuestItemDeliveryHandler
    - if <[InProgress]>:
        - narrate format:StoicSageFormat <proc[ItemDescriptionHandler].context[<[delivery_item]>]>

UnlockNatureboltQuestExperienceDeliveryHandler:
    type: task
    debug: false
    script:
    - define stage:2
...
UnlockNatureboltCompletion:
    type: task
    debug: false
    script:
    - execute as_server "cast teach <player.name> Naturebolt"