...
arcblade_chain:
    ...
    events:
        ...
        - if <context.skill> == "arcblade-task-runner":
            ...
            - if <[L1].size> == 0 queue clear
            - define L1_dist <[L1].last.location.distance[<c.target.location>]>
            - define L2 li@
            - foreach <[L1]>:
                - define L2 <[L2].include[<[value].location.find.entities.within[10].exclude[<[L1].include[<c.caster>|<c.target>]>].filter[location.distance[<c.target.location>].is[MORE].than[<[L1_dist]>]]>]>
            - define L2 <[L2].deduplicate.get[1].to[6]>
            - foreach <[L1]>:
                - define ent <[value]>
                - playeffect <c.target.location.points_between[<[ent].location>].distance[0.1]> effect:villager_happy offset:0,0,0
                - repeat 2:
                    - if <[L2].size> == 0 queue clear
