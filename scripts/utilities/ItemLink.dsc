...
link_messagetodiscord:
    ...
    - if !<[item].flags.contains[HIDE_ENCHANTS]>:
        ...
        - foreach <[item].enchantments.with_levels>:
            - define ench_list '<[ench_list].include[<s@translation_data.data_key[enchantments.<[value].before[,]>]> <[value].after[,]>]>'
    ...
    - if !<[item].flags.contains[HIDE_ATTRIBUTES]>:
        ...
        - foreach <[item].nbt_attributes>:
            ...
            - define attrb_list '<[attrb_list].include[<tern[<[atr_value].is[OR_MORE].than[0]>]:+||-><[atr_value]><tern[<[operation].is[EQUALS].to[0]>]:||<&pc>> <s@translation_data.data_key[attribute.<[name]>]> (<[slot].to_titlecase>)]>'
