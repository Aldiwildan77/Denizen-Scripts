UnlockAquaboltQuestDeliveryHandler:
    type: task
    debug: false
    script:
    - define stage:1
    - define quest_internalname:UnlockAquabolt
    - choose <player.item_in_hand.material.name>:
        ...
        - case default:
            ...
            - run QuestProgressHandler def:UnlockAquabolt
            - stop
    - inject QuestItemDeliveryHandler
    - if <[InProgress]>:
        - narrate format:StoicSageFormat <proc[ItemDescriptionHandler].context[<[delivery_item]>]>

UnlockAquaboltQuestExperienceDeliveryHandler:
    type: task
    debug: false
    script:
    - define stage:2
...
UnlockAquaboltCompletion:
    type: task
    debug: false
    script:
    - execute as_server "cast teach <player.name> Aquabolt"