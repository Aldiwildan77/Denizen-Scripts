UnlockWindboltQuestDeliveryHandler:
    type: task
    debug: false
    script:
    - define stage:1
    - define quest_internalname:UnlockWindbolt
    - choose <player.item_in_hand.material.name>:
        ...
        - case default:
            ...
            - run QuestProgressHandler def:UnlockWindbolt
            - stop
    - inject QuestItemDeliveryHandler
    - if <[InProgress]>:
        - narrate format:StoicSageFormat <proc[ItemDescriptionHandler].context[<[delivery_item]>]>

UnlockWindboltQuestExperienceDeliveryHandler:
    type: task
    debug: false
    script:
    - define stage:2
...
UnlockWindboltCompletion:
    type: task
    debug: false
    script:
    - execute as_server "cast teach <player.name> Windbolt"